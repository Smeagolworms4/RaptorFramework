...
class RaptorDAO {
	...
	public static function getInstance ($tablename, $jointures = array (), $connectionName = NULL) {
		...
		$classname = 'RaptorDAO_'.$tablename.'_'.$connectionName;
		$JOIN = '';
		$joinParse = array ();
		...
		if (!isset (self::$_instances[$classname])) {
			...
			if (!file_exists ($file)) {
				...
				$db = RaptorDB::getConnection ($connectionName);
				$tables = $db->getTableList ();
				
				if (!in_array($tablename, $tables)) {
					throw new RaptorDAOException (__('La table \'%0\' n\'existe pas dans la connexion \'%1\'', array ($tablename, $connectionName)));
				}
				...
				foreach ($joinParse as $key=>$j) {
					if (!in_array($j[0], $tables)) {
						throw new RaptorDAOException (__('La table \'%0\' n\'existe pas dans la connexion \'%1\'', array ($j[0], $connectionName)));
					}
		...
		if ($listSelect) {
			...
			foreach ($listSelect as $key=>$value) {
				...
				if ($this->tableNameByAlias ($infos[0]) === NULL) {
					throw new RaptorDAOException (__('L\'alias %0 ne correspond à aucune jointure de la DAO', array ($infos[0])));
				}
				
				if (!$this->fieldExist ($name)) {
					throw new RaptorDAOException (__('Le champs \'%0\' n\'existe pas dans la table \'%1\'', array ($infos[1], $this->tableNameByAlias ($infos[0]))));
				}
	...
	public function get ($clefPrimaires, $listSelect = array ()) {
		...
		foreach ($this->_primarys as $key => $primary) {
			...
			if (!isset ($clefPrimaires[$key])) {
				throw new RaptorDAOException (__('Il manque des clef primaire la liste des clef demandé est la suivant \'%0\'', array (var_export($this->_primarys))));
			}
	 ...
	 */
	private function _validRecord ($record) {
		foreach ($this->_listeChamps as $field=>$value) {
			$record->$field = (isset ($record->$field)) ? $record->$field : NULL;
			...
			if (
				...
				throw new RaptorDAOException (__('Le champs \'%0\' de la table \'%1\' ne peut pas être NULL', array ($field, $this->_tablename)));
			}
		}
	}
	...
	public function insert ($record) {
		...
		foreach ($this->_listeChamps as $field=>$value) {
			...
			if ($record->$field === NULL && $value['notnull'] && strpos(strtolower ($value['extra']), 'on update') === false) {
				continue;
			}
			...
			$values[':'.$i] = (isset ($record->$field)) ? $record->$field : NULL;
	...
	public function update ($record) {
		...
		foreach ($this->_primarys as $primary) {
			$WHERE.='AND `'.$primary.'`=:'.$i.' ';
			$values[':'.$i] = (isset ($record->$primary)) ? $record->$primary : NULL;
			$i++;
		}
		$sql .= $WHERE;
