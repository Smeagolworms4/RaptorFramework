 ...
 */
class DB_PDOMySQL extends RaptorDB {
	...
	public function __construct ($host, $database, $user, $password) {
		try {
			...
			throw new DBException (__('Connexion à la base de donnée (host = \'%host\', database = \'%database\') impossible', array ('host'=>$host, 'database'=>$database)));
		}
	}
	...
	public function query ($sql, $params = array ()) {
		...
		if (!$stmt){
			throw new DBException (__('Impossible de préparer la requête [%0] - %1 - %2', array (
				$sql,
				serialize ($params),
		...
		if (!$stmt->execute ($params)){
			...
			throw new DBException (__('Impossible d\'exécuter la requête [%0] : %1', array (
				$sql,
		...
		@$stmt->setFetchMode(PDO::FETCH_CLASS, 'RaptorDAORecord');
	...
	public function getTableList () {
		...
		if ($this->_cacheListTables === NULL) {
			...
			foreach ($liste as $table) {
				$this->_cacheListTables[] = array_shift ($table->toArray ());
			}
		}
		...
		if (!isset ($this->_cacheListFields[$tableName])) {
			...
			foreach ($liste as $desc) {
				...
				switch ($field->type) {
					...
					default:
						throw new DBException(__('Le type \'%0\' du champs \'%1\' de la table \'%2\' n\'est pas géré', array ($field->type, $desc->Field, $tableName)));
				}
